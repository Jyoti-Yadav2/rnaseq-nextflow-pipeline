...
The final report generated by the completed run is `results_final/multiqc/multiqc_report.html`. It is currently stored on the analysis machine and is not automatically public on GitHub because generated results are excluded by `.gitignore`.

To make the report public, upload this HTML file to a GitHub Release on this repository, or add it deliberately under a `docs/` folder and enable GitHub Pages. The report is based on public data, but check the ENA study's reuse terms and include the accession and citations wherever you share it.
